...
Returns the position of **Value** within `[RangeMin, RangeMax]` as a normalized fraction in `[0, 1]`. This is equivalent to mapping to an output range of `[0, 1]` and is useful for converting an absolute value into a proportion â€” for example turning a remaining health value into a 0-to-1 bar fill amount.
