...
Calls a `BlueprintPure` function on an actor (or one of its components) at runtime and exposes the return value as a bindable `FName` output. Use this to retrieve name-typed results from Blueprint logic â€” for example a socket name, a state identifier, or a tag key.
