...
Calls a `BlueprintPure` function on an actor (or one of its components) at runtime and exposes the return value as a bindable `int64` output. Use this when the function returns a large integer that exceeds the range of a standard 32-bit integer â€” for example a timestamp, a unique ID, or an accumulated score.
