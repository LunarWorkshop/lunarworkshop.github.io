...
Calls a `BlueprintPure` function on an actor (or one of its components) at runtime and exposes the return value as a bindable `int32` output. Use this to retrieve integer values from Blueprint pure functions â€” for example a count, an index, or a discrete state identifier.
