...
Draws a solid-filled debug box defined by an `FBox` (min/max bounds) and a world-space `FTransform`. When **While Task Is Active** is enabled the box is redrawn every tick. When disabled it is drawn once on enter and the task immediately succeeds. For an extent-based solid box see [Draw Debug Solid Box](/StateTreeTools/tasks/draw-debug-solid-box).
