...
Draws a solid-filled axis-aligned debug box defined by a center point and half-extents. When **While Task Is Active** is enabled the box is redrawn every tick. When disabled it is drawn once on enter and the task immediately succeeds. For a rotated variant see [Draw Debug Solid Box (Rotated)](/StateTreeTools/tasks/draw-debug-solid-box-rotated).
