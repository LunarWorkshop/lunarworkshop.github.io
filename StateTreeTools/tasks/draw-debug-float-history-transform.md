...
Draws a debug graph of a `FDebugFloatHistory` positioned and oriented by a full `FTransform`. The graph shows the recorded float values over time as a 2D chart in world space. When **While Task Is Active** is enabled the graph is redrawn every tick. When disabled it is drawn once on enter and the task immediately succeeds. For a location-only version see [Draw Debug Float History](/StateTreeTools/tasks/draw-debug-float-history).
