...
Draws a smooth centripetal Catmull-Rom spline through an array of world-space control points, with a separate color specified per segment. When **While Task Is Active** is enabled the spline is redrawn every tick. When disabled it is drawn once on enter and the task immediately succeeds. For a single-color variant see [Draw Centripetal Catmull-Rom Spline](/StateTreeTools/tasks/draw-centripetal-catmull-rom-spline).
