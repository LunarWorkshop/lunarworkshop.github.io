...
Draws a solid-filled oriented debug box defined by a center point, half-extents, and a quaternion rotation. When **While Task Is Active** is enabled the box is redrawn every tick. When disabled it is drawn once on enter and the task immediately succeeds. For an axis-aligned variant see [Draw Debug Solid Box](/StateTreeTools/tasks/draw-debug-solid-box).
