...
By default the event is sent when the state is **entered**. You can also send it on **exit**, with separate toggles for which exit reasons trigger the send (Stopped, Succeeded, or Failed). See [Enter / Exit Firing](/StateTreeTools/enter-exit-firing) for a full explanation.
