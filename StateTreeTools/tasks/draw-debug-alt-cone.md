...
Draws a wireframe debug cone defined by an origin, a rotator, a length, and angular widths. This is an alternative to [Draw Debug Cone](/StateTreeTools/tasks/draw-debug-cone) that uses a `FRotator` instead of a direction vector to orient the cone. When **While Task Is Active** is enabled the cone is redrawn every tick. When disabled it is drawn once on enter and the task immediately succeeds.
