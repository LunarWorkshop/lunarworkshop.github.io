...
Draws a debug circle oriented by a full transformation matrix. When **While Task Is Active** is enabled the circle is redrawn every tick. When disabled it is drawn once on enter and the task immediately succeeds. For a simpler center+axis-vector version see [Draw Debug Circle](/StateTreeTools/tasks/draw-debug-circle).
