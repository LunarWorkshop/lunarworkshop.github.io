...
Draws a debug circle in 3D space defined by a base position and explicit X and Y axis vectors, giving full control over the circle's plane orientation. When **While Task Is Active** is enabled the circle is redrawn every tick. When disabled it is drawn once on enter and the task immediately succeeds. For a center+YZ-axis version see [Draw Debug Circle](/StateTreeTools/tasks/draw-debug-circle).
