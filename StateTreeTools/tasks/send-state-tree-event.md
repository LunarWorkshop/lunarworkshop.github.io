...
### Payload
An `FInstancedStruct` that carries typed data with the event. Pick any struct type â€” its fields appear inline in the editor as the payload.
...
By default the event is sent when the state is **entered**. You can also send it on **exit**, with fine-grained control over which exit reasons trigger the send (Stopped, Succeeded, or Failed). See [Enter / Exit Firing](/StateTreeTools/enter-exit-firing) for a full explanation.
...
[â† Back to Gameplay \| StateTree](/StateTreeTools/tasks/statetree) Â· [â† Back to home](/StateTreeTools/)
