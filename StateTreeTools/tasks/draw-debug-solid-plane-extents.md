...
Draws a solid-filled debug plane centered at a location, using separate X and Y extents to control its dimensions. When **While Task Is Active** is enabled the plane is redrawn every tick. When disabled it is drawn once on enter and the task immediately succeeds. For a version with a single uniform size see [Draw Debug Solid Plane](/StateTreeTools/tasks/draw-debug-solid-plane).
