...
Draws a solid-filled debug plane centered at a location, using a uniform size for both axes. When **While Task Is Active** is enabled the plane is redrawn every tick. When disabled it is drawn once on enter and the task immediately succeeds. For a version with separate X/Y extents see [Draw Debug Solid Plane (Extents)](/StateTreeTools/tasks/draw-debug-solid-plane-extents).
