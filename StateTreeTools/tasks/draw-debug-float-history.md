...
Draws a debug graph of a `FDebugFloatHistory` at a fixed world-space location. The graph shows the recorded float values over time as a 2D chart in world space. When **While Task Is Active** is enabled the graph is redrawn every tick. When disabled it is drawn once on enter and the task immediately succeeds. For a version positioned by a full `FTransform` see [Draw Debug Float History (Transform)](/StateTreeTools/tasks/draw-debug-float-history-transform).
