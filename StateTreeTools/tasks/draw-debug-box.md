...
Draws an axis-aligned debug box centered at a world-space position. When **While Task Is Active** is enabled the box is redrawn every tick. When disabled it is drawn once on enter and the task immediately succeeds. For a rotated box see [Draw Debug Box (Rotated)](/StateTreeTools/tasks/draw-debug-box-rotated).
...
[â† Back to Debug](/StateTreeTools/tasks/debug) Â· [â† Back to home](/StateTreeTools/)
