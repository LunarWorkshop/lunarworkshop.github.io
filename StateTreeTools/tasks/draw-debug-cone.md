...
Draws a wireframe debug cone defined by an origin, a direction vector, a length, and separate angular widths for horizontal and vertical spread. Angles are specified in radians. When **While Task Is Active** is enabled the cone is redrawn every tick. When disabled it is drawn once on enter and the task immediately succeeds. For a cone defined by origin and rotation see [Draw Debug Alt Cone](/StateTreeTools/tasks/draw-debug-alt-cone).
